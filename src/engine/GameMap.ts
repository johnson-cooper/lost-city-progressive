...
export function botFindPath(level: number, srcX: number, srcZ: number, destX: number, destZ: number): Uint32Array {
    return rsmod.findPath(level, srcX, srcZ, destX, destZ, 1, 1, 1, 0, -1, false, 0, -1, CollisionType.NORMAL);
}

/** Bot pathfinding with unlimited BFS iterations (maxWaypoints=-1) — routes around any obstacle. */
export function botWalkPath(level: number, srcX: number, srcZ: number, destX: number, destZ: number): Uint32Array {
    return rsmod.findPath(level, srcX, srcZ, destX, destZ, 1, 1, 1, 0, -1, true, 0, -1, CollisionType.NORMAL);
}
